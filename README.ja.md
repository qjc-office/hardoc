...
HarDoc は Claude Code と Codex のハーネスを点検します。まず報告し、設定を変えるのはあなたが承認したあとだけです。
...
HarDoc はあなたの承認なしに何も変更しません。`skill-governor` skill は読み取り専用です。`trim` skill はプレビューを承認したあとにだけ変更を適用し、先にスナップショットを取り、元に戻すためのコマンドを 1 行表示します。
