...
Cross-check whichever listing and status surfaces the installed version actually supports: `claude mcp list` or `codex mcp list`, the plugin listing commands, and Claude Code's `/skill-doctor`, `/context` and `/mcp`, or Codex's `/skills` and `/mcp`. Confirm each one against the installed help rather than assuming it exists. Listing MCP servers can start processes or open connections, so do this only in a trusted configuration, and never auto-accept an authentication, trust or modification prompt. When a project-scoped server's execution approval is unconfirmed, or the configuration's trust state is unclear, skip connection-based listing, parse settings files only, and leave connection state `unknown`. Presence in a file or a list is not approval to execute, and registration alone is not evidence of a successful connection, exposed tools or a successful call.
