...
이 스킬 단위 설정은 스킬 폴더에서 읽힌 스킬에만 닿습니다. 플러그인이 제공한 스킬은 이 설정을 무시하므로, 그쪽 레버는 플러그인 통째뿐입니다. `trim`은 아무 효과도 없을 설정을 조용히 써넣는 대신 그 사실을 알려줍니다. HarDoc 자신도 플러그인이라 자기 스킬 두 개에 같은 제약이 걸립니다.
